...
function _ver(artistver) {
  return (
    Object.keys(artistver[0])[3]
  )
}

function _puBlic(artistpublic) {
  return (
    Object.keys(artistpublic[0])[4]
  )
}

function _ver_col(artistver, ver) {
  return (
    artistver.map(row => row[ver])
  )
}

function _public_col(artistpublic, puBlic) {
  return (
    artistpublic.map(row => row[puBlic])
  )
}

function _value(ver_col) {
  return (
    [...new Set(ver_col)].sort()
  )
}

function _ver_total(value, ver_col) {
  return (
    value.map(val => ({
      value: val,
...
function _public_total(value, public_col) {
  return (
    value.map(val => ({
      value: val,
...
function _data(ver_total, public_total) {
  return (
...
function _14(Plot, ver, data, selectedSeries) {
  return (
    Plot.plot({
      height: 600,
      title: ver,
...
function _selectedSeries1(Inputs) {
  return (
    Inputs.checkbox(["artist", "artistpublic"], { label: "Choose datasets", value: ["artist", "artistpublic"] })
  )
}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("puBlic")).define("puBlic", ["artistpublic"], _puBlic);
  main.variable(observer("ver_col")).define("ver_col", ["artistver", "ver"], _ver_col);
  main.variable(observer("public_col")).define("public_col", ["artistpublic", "puBlic"], _public_col);
