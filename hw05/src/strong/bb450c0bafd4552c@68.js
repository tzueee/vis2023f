...
function _ten(FileAttachment) {
  return (
    FileAttachment("ten@2.json").json()
  )
}

function _strong(d3, ten, drag, url_list, invalidation) {
  // 指定圖表的尺寸。
  const width = 1000;
  const height = 500;

  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(ten);
  ...
  const imageRadius = circleRadius + 15; // 圖片環繞外框的半徑
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["ten@2.json", { url: new URL("../ten.json", import.meta.url), mimeType: "application/json", toString }]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("ten")).define("ten", ["FileAttachment"], _ten);
  main.variable(observer("strong")).define("strong", ["d3", "ten", "drag", "url_list", "invalidation"], _strong);
