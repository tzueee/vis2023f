...
function _innerQ(artist) {
  return (
    Object.keys(artist[0])[2]
  )
}

function _outerQ(artist) {
  return (
...
<h3>從上圖中，我們可以看出：
  <ul>
    <li>工作地點在「北部」占此問卷的多數</li>
    <li>大多數的工作處有在以行動或計畫進行永續事務</li>
    <li>所有資料中，比起有推動永續事務，「沒有」推動較「不清楚」有沒有推動的多</li>
  </ul>
</h3>`
  )
}
...
function _color(d3) {
  return (
    d3
      .scaleOrdinal()
      .domain(["北部", "中部", "南部", "東部", "有，以行動或計畫進行", "不清楚", "沒有"])
      //.range(d3.schemePaired)
      .range(["#DC9FB4", "#51A8DD", "#6c5a8f", "#B5CAA0", "#5ecc65", "#e3924b", "#ed5a5a"])
      .unknown("#BEBEBE")
  )
}
