...
function _bgColor(Inputs) {
  return (
    Inputs.color({ label: "background color", value: "#475866" })
  )
}
...
function _LivingPlace(data) {
  return (
    Object.keys(data[0])[0]
  )
}

function _LivingPlace_column(data, LivingPlace) {
  return (
    data.map(row => row[LivingPlace])
  )
}

function _AllLivingCity(LivingPlace_column) {
  return (
    [...new Set(LivingPlace_column)].sort()
  )
}

function _counts(AllLivingCity, LivingPlace_column) {
  return (
    AllLivingCity.map(val => ({
      value: val,
      count: LivingPlace_column.filter(v => v === val).length
    }))
  )
}

function _minidata(counts) {
  return (
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("LivingPlace")).define("LivingPlace", ["data"], _LivingPlace);
  main.variable(observer("LivingPlace_column")).define("LivingPlace_column", ["data", "LivingPlace"], _LivingPlace_column);
  main.variable(observer("AllLivingCity")).define("AllLivingCity", ["LivingPlace_column"], _AllLivingCity);
  main.variable(observer("counts")).define("counts", ["AllLivingCity", "LivingPlace_column"], _counts);
  main.variable(observer("minidata")).define("minidata", ["counts"], _minidata);
